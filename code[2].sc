	...
	/////////////////////////////////////////////////////////////
	//
	//code[2].sc / data[2].xqd
	//   By this Scilab code, you can convert an SPM data file
	//   of Hitachi High-Tech Science Corporation's into ISO data format.
	...
	[fname, dpath, findex] = uigetfile({'*.xqd',  ' (*.xqd)'});
	...
	if findex==0 
		...
		mseek(1402,fd1,'set');
		im_xsiz=mget(1,'sl',fd1);

		mseek(1404,fd1,'set');
		im_ysiz=mget(1,'sl',fd1);

		mseek(152,fd1,'set');
		xlength=mget(1,'dl',fd1)*1.0D+3;

		mseek(160,fd1,'set');
		ylength=mget(1,'dl',fd1)*1.0D+3;
